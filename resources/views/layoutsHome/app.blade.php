...
<body class="flex h-screen overflow-hidden bg-[#e9f2fb]">
    ...
    <aside class="w-60 flex flex-col flex-shrink-0 border-r border-[#c0d8ec]" style="background-color: var(--sidebar-bg);">
        ...
        @php
            ...
            $menus = [
                ['route' => $prefix . '.data_pbpd',       'label' => 'DATA PB/PD ULP',    'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m5.231 13.481L15 17.25m-4.5-15H5.625c-.621 0-1.125.504-1.125 1.125v16.5c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9zm3.75 11.625a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />'],
                ['route' => $prefix . '.tanpa_perluasan', 'label' => 'TANPA PERLUASAN',   'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15" />'],
                ['route' => $prefix . '.perluasan_jtm',   'label' => 'PERLUASAN JTM',     'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 12c0-1.232-.046-2.453-.138-3.662a4.006 4.006 0 00-3.7-3.7 48.678 48.678 0 00-7.324 0 4.006 4.006 0 00-3.7 3.7c-.017.22-.032.441-.046.662M19.5 12l3-3m-3 3l-3-3m-12 3c0 1.232.046 2.453.138 3.662a4.006 4.006 0 003.7 3.7 48.656 48.656 0 007.324 0 4.006 4.006 0 003.7-3.7c.017-.22.032-.441.046-.662M4.5 12l3 3m-3-3l-3 3" />'],
                ['route' => $prefix . '.perluasan_jtr',   'label' => 'PERLUASAN JTR',     'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />'],
                ['route' => $prefix . '.restitusi',        'label' => 'RESTITUSI',          'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3" />'],
                ['route' => $prefix . '.pengoperasian',   'label' => 'PENGOPERASIAN',     'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" />'],
                ['route' => $prefix . '.pencarian',       'label' => 'PENCARIAN',          'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />'],
                ['route' => $prefix . '.laporan',         'label' => 'LAPORAN',            'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />'],
                ['route' => $prefix . '.notifikasi',      'label' => 'NOTIFIKASI',         'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" />'],
            ];
        @endphp
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header class="bg-white border-b border-slate-200 flex-shrink-0" style="background-color: var(--header-blue);">
            <div class="flex items-stretch" style="background:white; min-height:56px;">
                {{-- Yellow PLN bolt --}}
                <div class="flex-shrink-0 bg-[#fff500] px-3 flex items-center">
                    <svg viewBox="0 0 50 70" xmlns="http://www.w3.org/2000/svg" class="w-8 h-10">
                        <path d="M 25 5 L 8 30 L 22 30 L 15 50" fill="none" stroke="#00a2e9" stroke-width="7" stroke-linecap="square"/>
                        <path d="M 40 40 L 24 40 L 30 22" fill="none" stroke="#ed1c24" stroke-width="7" stroke-linecap="square"/>
                    </svg>
                </div>
                {{-- Header Text --}}
