...
<div class="h-full flex flex-col bg-slate-50 p-5">
    ...
    <tr class="bg-[#0D1B8C] text-white">
    <th class="border border-blue-600 px-3 py-1.5 text-center font-medium">KTP</th>
    <th class="border border-blue-600 px-3 py-1.5 text-center font-medium">IJIN</th>
...
<td class="border border-slate-200 px-3 py-2 text-center">
    <button onclick="openSyaratModal({{ json_encode($item) }}, 'ktp')" class="text-slate-400 hover:text-emerald-600 transition" title="Lihat KTP">
...
<td class="border border-slate-200 px-3 py-2 text-center">
    ...
    <td class="border border-slate-200 px-2 py-2 text-center text-xs text-slate-500 whitespace-nowrap">{{ $item->tanggal_bayar ? \Carbon\Carbon::parse($item->tanggal_bayar)->format('d/m/Y') : '-' }}</td>
    <td class="border border-slate-200 px-3 py-2 text-center">{{ $item->durasi_hari_kerja ?? '-' }}</td>
</tr>
                        @empty
                        <tr id="emptyRow">
                            <td colspan="17" class="text-center py-12 text-slate-400 italic text-xs">
    ...
    <div class="bg-white rounded shadow-2xl w-full max-w-3xl relative border border-slate-300 overflow-hidden">
        ...
        <div class="px-6 pt-10 pb-5 text-[11px] text-slate-800 font-mono">
            ...
            <div class="grid grid-cols-[160px_1fr] gap-y-[3px] mb-4">
                <div class="font-semibold uppercase text-slate-500">RAB</div>
            ...
            <table class="w-full border-collapse border border-slate-400 text-[11px] text-center">
                <thead>
                    <tr class="bg-red-200">
                        <th colspan="2" class="border border-slate-400 py-1.5 uppercase font-bold tracking-wide text-slate-700">
                            PERSYARATAN PROSES FASTON 360°
                        </th>
                    </tr>
                    <tr class="bg-slate-50">
                        <th class="border border-slate-400 py-1 px-3 uppercase font-semibold w-1/2">UPLOAD KTP</th>
                        <th class="border border-slate-400 py-1 px-3 uppercase font-semibold w-1/2">UPLOAD IJIN TANAM TIANG</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        ...
                        <td class="border border-slate-400 px-4 py-2">
                            <div class="flex items-center justify-center gap-2">
                                <label class="flex items-center gap-1 cursor-pointer">
                                    ...
                                    <span class="px-2 py-0.5 bg-slate-100 border border-slate-400 rounded text-[10px] hover:bg-slate-200 transition cursor-pointer" onclick="document.getElementById('syr-file-ktp').click()">Choose File</span>
                                    <span id="syr-ktp-filename" class="text-[10px] text-slate-400 italic">No file chosen</span>
                                </label>
                                <button onclick="uploadSyarat('ktp')" class="px-2 py-0.5 bg-[#2B73FE] text-white rounded text-[10px] hover:bg-blue-700 transition">Upload KTP</button>
                            </div>
                        </td>
                        <!-- IJIN Upload -->
                        <td class="border border-slate-400 px-4 py-2">
                            <div class="flex items-center justify-center gap-2">
                                <label class="flex items-center gap-1 cursor-pointer">
                                    ...
                                    <span class="px-2 py-0.5 bg-slate-100 border border-slate-400 rounded text-[10px] hover:bg-slate-200 transition cursor-pointer" onclick="document.getElementById('syr-file-itt').click()">Choose File</span>
                                    <span id="syr-itt-filename" class="text-[10px] text-slate-400 italic">No file chosen</span>
                                </label>
                                <button onclick="uploadSyarat('itt')" class="px-2 py-0.5 bg-[#2B73FE] text-white rounded text-[10px] hover:bg-blue-700 transition">Upload ITT</button>
                            </div>
                        </td>
                    </tr>
                    <tr class="bg-slate-50">
                        <td class="border border-slate-400 py-1.5 text-slate-500" id="syr-ktp-count">0</td>
                        <td class="border border-slate-400 py-1.5 text-slate-500" id="syr-itt-count">0</td>
                    </tr>
                </tbody>
            </table>
...
function openSyaratModal(item, tipe) {
    ...
    document.getElementById('syr-transaksi').textContent = (item.transaksi || '-') + ' - ' + (item.transaksi || '-');
    ...
    document.getElementById('syr-nama').textContent     = item.nama_pelanggan || ('PELANGGAN ' + (item.no_agenda || ''));
    ...
    document.getElementById('syr-rab').textContent      = '12850800';
...
function uploadSyarat(tipe) {
    var input = document.getElementById('syr-file-' + tipe);
    var countEl = document.getElementById('syr-' + tipe + '-count');
    if (!input.files || !input.files[0]) {
    ...
    alert('File berhasil diupload: ' + input.files[0].name);
    input.value = '';
    document.getElementById('syr-' + tipe + '-filename').textContent = 'No file chosen';
}
</script>
