        ...
        <div class="bg-white rounded-2xl shadow-md border border-slate-200 flex flex-col flex-1 min-h-0 overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-[#0D1B8C] to-[#0D1B8C] px-5 py-2.5 flex items-center justify-between flex-shrink-0">
                <div class="flex items-center gap-2">
                    <span class="text-white font-bold text-sm tracking-wide">DATA TRANSAKSI PERLUASAN JTM</span>
        ...
        <div class="min-h-0 overflow-y-auto p-6 text-[13.5px] text-slate-800 font-mono flex flex-col gap-4">
            ...
            <div id="rab-section_jtm" class="grid grid-cols-[150px_1fr] gap-2 items-center bg-blue-50/50 p-3 rounded-lg border border-blue-100">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="number" id="jtm-mdl-rab-input" class="border border-slate-300 rounded-lg px-3 py-1.5 text-sm w-40 focus:outline-none focus:ring-2 focus:ring-[#2B73FE] transition bg-white" placeholder="0" @if (!str_starts_with(Auth::user()->role ?? '', 'managerULP') && (Auth::user()->role ?? '') !== 'perencanaan') readonly @endif />
            ...
            <div id="expansion-detail-section_jtm" class="bg-orange-50/60 border border-orange-200 rounded-lg p-3">
                ...
                <div class="overflow-hidden rounded-lg border border-slate-300 bg-white">
                    <table class="w-full border-collapse text-[11px]"><thead class="bg-slate-100 text-slate-600"><tr><th class="border-b border-slate-300 px-2 py-2 text-left">KETERANGAN</th><th class="border-b border-l border-slate-300 px-2 py-2 text-left">DROPDOWN</th><th class="border-b border-l border-slate-300 px-2 py-2 text-left">JUMLAH</th><th class="border-b border-l border-slate-300 px-2 py-2 text-left">SATUAN</th></tr></thead><tbody>
                        ...
                        {{-- penambahan untuk kwh meter yang terbaru --}}
                        <tr><td class="border-b border-slate-200 px-2 py-2 font-bold">JUMLAH KWH METER</td><td class="border-b border-l border-slate-200 px-2 py-2"></td><td class="border-b border-l border-slate-200 px-2 py-2"><input id="jtm-mdl-jumlah-kwh" type="text" class="w-full border border-slate-300 rounded px-2 py-1.5 bg-orange-50" @if (!str_starts_with(Auth::user()->role ?? '', 'managerULP')) readonly @endif></td><td class="border-b border-l border-slate-200 px-2 py-2">METER</td></tr>
                    </tbody></table>x
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div id="wo-controls_jtm" class="flex flex-col gap-1.5 bg-white p-2.5 rounded-lg border border-amber-200">
                        <div class="flex items-center justify-between">
                            ...
                            <label id="jtm-mdl-wo-upload" class="cursor-pointer bg-amber-600 hover:bg-amber-700 text-white text-[10.5px] font-bold px-2 py-1 rounded shadow-sm transition">
                                Upload WO
...
function openDetailModal_jtm(item) {
    ...
    var ulpStatus = document.getElementById('jtm-mdl-status-ulp-kirim');
    if (ulpStatus) {
...
function renderExpansionDetail_jtm(item) {
    ...
    document.getElementById('jtm-mdl-jumlah-kwh').value = detail.jumlah_kwh || '';
}
...
function saveExpansionDetail_jtm() {
    ...
    var detail = {
        ...
        jumlah_kwh: document.getElementById('jtm-mdl-jumlah-kwh').value.trim()
    };
    fetch('/' + _apiPrefix + '/api/simpan-detail-perluasan', {method:'POST', headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'}, body:JSON.stringify({agendaKey: window.currentItem_jtm.agendaKey || window.currentItem_jtm.no_agenda, dest:'jtm', detail_perluasan:detail})})
...
function updateVendorReportUploadState_jtm(noAgenda) {
    var enabled = hasVendorReport_jtm(noAgenda);
    var controls = ['jtm-mdl-wo-upload', 'jtm-mdl-dokumen-upload'];
    ...
    var note = document.getElementById('jtm-vendor-report-upload-note');
...
function handleWoUpload_jtm(input) {
    if (!window.currentItem_jtm) return;
    if (!hasVendorReport_jtm(window.currentItem_jtm.no_agenda)) {
    ...
    files.forEach(function(file) {
        uploadServerBerkas_jtm(window.currentItem_jtm.no_agenda, agendaKey, 'wo_perencanaan', file);
    });
...
function handleBaCekUpload_jtm(input) {
    if (!window.currentItem_jtm) return;
    var files = Array.from(input.files);
    if (!files.length) return;
    ...
    var agendaKey = window.currentItem_jtm.no_agenda || 'default';
    files.forEach(function(file) { uploadServerBerkas_jtm(window.currentItem_jtm.no_agenda, agendaKey, 'ba_cek', file); });
    Promise.all(files.map(function(file) { return new Promise(function(resolve) {
        ...
        var req = indexedDB.open('FastOnDocs', 2); req.onsuccess = function(e) {
            ...
            gr.onsuccess = function() { var d = gr.result || {}; d.agendaKey = agendaKey; d.ba_cek = (d.ba_cek || []).concat(newFiles); store.put(d); tx.oncomplete = function() { renderBaCekList_jtm(agendaKey); }; };
        };
    });
    input.value = '';
}

function renderBaCekList_jtm(agendaKey) {
    var req = indexedDB.open('FastOnDocs', 2); req.onsuccess = function(e) {
        var tx = e.target.result.transaction('uploadedDocs', 'readonly'), gr = tx.objectStore('uploadedDocs').get(agendaKey);
        gr.onsuccess = function() {
            var list = document.getElementById('jtm-mdl-ba-cek-list'), localFiles = (gr.result && gr.result.ba_cek) || [];
            var files = ((window.currentItem_jtm && window.currentItem_jtm.berkas) || []).filter(function(file) { return file.jenis_berkas === 'ba_cek'; }).map(function(file) { return { id: file.id, name: file.nama_file, path: file.path_file }; });
...
function deleteBaCek_jtm(id, fileName, agendaKey) {
    ...
    fetch('/' + _currentRole + '/api/berkas/' + id + '/ba-cek', { method: 'DELETE', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } })
        .then(function(r) { return r.json(); }).then(function(res) { if (!res.success) throw new Error(res.message); window.currentItem_jtm.berkas = (window.currentItem_jtm.berkas || []).filter(function(file) { return file.id !== id; }); removeLocalBaCek_jtm(agendaKey, fileName); })
        .catch(function(err) { alert(err.message || 'Gagal menghapus BA Cek.'); });
}

function removeLocalBaCek_jtm(agendaKey, fileName) {
    var req = indexedDB.open('FastOnDocs', 2); req.onsuccess = function(e) {
        var db = e.target.result; if (!db.objectStoreNames.contains('uploadedDocs')) { renderBaCekList_jtm(agendaKey); return; }
        var tx = db.transaction('uploadedDocs', 'readwrite'), store = tx.objectStore('uploadedDocs'), gr = store.get(agendaKey);
        gr.onsuccess = function() { var d = gr.result; if (d && d.ba_cek) { d.ba_cek = d.ba_cek.filter(function(file) { return file.name !== fileName; }); store.put(d); } tx.oncomplete = function() { renderBaCekList_jtm(agendaKey); }; };
    };
}
...
function handleExcelUpload_jtm(input) {
    ...
    files.forEach(function(file) {
        uploadServerBerkas_jtm(window.currentItem_jtm.no_agenda, agendaKey, 'dokumen', file);
    });
